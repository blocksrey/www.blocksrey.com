...
for (let spec in spec_group){
	...
	i_group[grouptot]=spec_group[spec]
	++grouptot
	for (i in spec_group[spec])
		n_bool[spec_group[spec][i]]=1
}

function randint(n){return floor(n*rand())}
function randarr(a){return a[randint(a.length)]}
function randomselect(){return randarr(i_group[randint(grouptot)])}

function randomselectiters(n){
	if(n_bool[n]){
		let iter=0
		while(1){
			++iter
