...
for f in *.ico; do
	n=`basename $f .ico`
	ffmpeg -loglevel warning -y -i $f -lossless 1 -vf scale=-1:16:flags=neighbor ../icons/$n.gif
done
...
for f in *.gif; do
	n=`basename $f .gif`
	ffmpeg -loglevel warning -y -i $f -lossless 1 -vf scale=-1:32:flags=neighbor ../icons/$n.gif
done